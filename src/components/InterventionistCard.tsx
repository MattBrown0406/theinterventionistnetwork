    ...
    <div className="bg-card border border-border rounded-lg p-6 hover:shadow-lg transition-shadow group">
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {specialties.slice(0, 3).map((spec) => (
          <span key={spec} className="text-xs px-2 py-0.5 rounded-full bg-gold-light text-gold-foreground font-medium">
            {spec}
          </span>
        ))}
      </div>
      ...
      {!compact && (
        <>
          ...
          {quote && (
            <p className="text-xs italic text-muted-foreground mb-4 border-l-2 border-gold pl-3">
              "{quote}"
            </p>
          )}
        </>
